...
### 为什么需要可撤销？
在现实中（如员工激励），公司需要在员工离职时收回 **未归属** 的代币，而标准 Vesting 合约通常不支持或操作复杂。
...
### Foundry 与不变量测试（Invariant Testing）
我们选择 Foundry 进行测试，因为其执行速度快，并且支持 **不变量测试**。

### 为什么进行不变量测试？
单元测试验证特定路径，但不变量测试验证在**任何调用顺序下都必须始终成立的属性**。
...
在不变量测试 `invariant_Solvency` 中，我们验证：
